...
class Agent(object):
    def __init__(self, gamma, epsilon, alpha, n_actions, input_dims,
                 mem_size, batch_size, eps_min=0.01, eps_dec=0.99998,
    ...
    def choose_action(self, observation, state, targets):
        if np.random.random() > self.epsilon:
            observation = T.tensor(observation, dtype=T.float)
            observation = observation[np.newaxis,:]
            curr_state = T.tensor(observation).to(self.q_eval.device)
            _, advantage = self.q_eval.forward(curr_state)
            action = T.argmax(advantage).item()
            res = []
            #print(advantage[0])
            for c,s in enumerate(advantage[0]):
                #print(c)
                res.append((c,s))
            ...
            for _ in range(2*len(self.action_space)):
